...
import {
  documentUrl,
  getCollection,
  getDocumentOrNull as getDocument,
  listCollections,
  listDocuments,
  shouldIndexDocument,
  type OutlineDocument,
} from "@mort/core/kb/outline";
import { isSyncRunning, withSyncLock } from "@mort/core/kb/sync-lock";
import { rewriteAttachmentUrls } from "@mort/core/kb/attachments";
import { EVENTS_COLLECTION } from "@mort/core/kb/events-store";
...
/**
 * Whether a sync/reset is running — backed by a Postgres advisory lock (see
 * @mort/core/kb/sync-lock), not an in-memory flag, so it holds across
 * replicas rather than only guarding a single process.
 */
export function isFullSyncRunning(): Promise<boolean> {
...
export async function resetIndex(): Promise<{ docs: number; runs: number }> {
  ...
  if (result === null) throw new Error("A sync is running — wait for it to finish.");
  return result;
}
...
export async function fullSync(
  ...
  if (result === null) throw new Error("A full sync is already running.");
  return result;
}
