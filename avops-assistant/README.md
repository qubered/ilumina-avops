...
- **Chat** — streaming answers with a `kb_search` tool loop (Vercel AI SDK), plus provider-executed **web search** for general equipment/manufacturer info (`AI_WEB_SEARCH`, KB stays the only authority for venue-specific facts). Scope-guarded: the assistant declines anything that isn't venue AV/event ops and resists in-conversation override attempts. Citations render as typed source rows (wiki doc vs web link), multi-turn history, auto-titles.
