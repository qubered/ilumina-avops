 ...
 * Retry-After ourselves. Shared by the legacy normalise pipeline, the v1
 * decision pipeline and the agent loop.
 */
...
export async function withRateLimitRetry<T>(fn: () => Promise<T>): Promise<T> {
  for (let attempt = 1; ; attempt++) {
    try {
      ...
      console.warn(`[mort] rate-limited, waiting ${wait}s (attempt ${attempt}/${RATE_LIMIT_ATTEMPTS})`);
